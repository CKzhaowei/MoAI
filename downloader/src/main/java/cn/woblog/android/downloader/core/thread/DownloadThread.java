...
import cn.woblog.android.downloader.exception.DownloadOutOfMemException;
import cn.woblog.android.downloader.exception.DownloadPauseException;
import cn.woblog.android.downloader.util.FileDownloadUtils;
import cn.woblog.android.downloader.util.MemoryUtil;
...
public class DownloadThread implements Runnable {
    ...
    private boolean isCheckedMem = false;

    public DownloadThread(DownloadThreadInfo downloadThreadInfo, DownloadResponse downloadResponse,
                          Config config,
                          DownloadInfo downloadInfo, DownloadProgressListener downloadProgressListener,int retryCount) {
        ...
        this.retryDownloadCount = retryCount;
    }
...
//    while (!(downloadInfo.isPause() || downloadThreadInfo.isThreadDownloadSuccess())) {
        ...
        checkPause();
        if (downloadInfo.getSize() > 0 && !isCheckedMem) {
            checkMemory();
            isCheckedMem = true;
        }
    try {
...
//          }
    ...
    private void checkMemory() {
        if ( downloadInfo.getSize() + MemoryUtil.SpareSpace > MemoryUtil.getAvailSpace(true)) {
            throw new DownloadOutOfMemException(DownloadException.EXCEPTION_OUTOFMEMORY);
        }
    }
    ...
    private void checkPause() {
        ...
        if (!config.getMobileCanDownload() &&  FileDownloadUtils.isNetworkNotOnWifiType()){
            throw new DownloadPauseException(DownloadException.EXCEPTION_PAUSE);
        }
    }
